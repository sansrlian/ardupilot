...
const AP_Param::GroupInfo NavEKF3::var_info[] = {
  ...
  // @Description: This enables EKF3. Enabling EKF3 only makes the maths run, it does not mean it
  // will be used for flight control. To use it for flight control set AHRS_EKF_TYPE=3. A reboot or
  // restart will need to be performed after changing the value of EK3_ENABLE for it to take effect.
  ...
  // @Description: This sets a lower limit on the speed accuracy reported by the GPS receiver that
  // is used to set horizontal velocity observation noise. If the model of receiver used does not
  // provide a speed accurcy estimate, then the parameter value will be used. Increasing it reduces
  // the weighting of the GPS horizontal velocity measurements.
  // @Range: 0.05 5.0
  // @Increment: 0.05
  // @User: Advanced
  // @Units: m/s
  ...
  // @Description: This sets a lower limit on the speed accuracy reported by the GPS receiver that
  // is used to set vertical velocity observation noise. If the model of receiver used does not
  // provide a speed accurcy estimate, then the parameter value will be used. Increasing it reduces
  // the weighting of the GPS vertical velocity measurements.
  // @Range: 0.05 5.0
  // @Increment: 0.05
  // @User: Advanced
  // @Units: m/s
  ...
  // @Description: This sets the percentage number of standard deviations applied to the GPS
  // velocity measurement innovation consistency check. Decreasing it makes it more likely that good
  // measurements will be rejected. Increasing it makes it more likely that bad measurements will be
  // accepted. If EK3_GLITCH_RAD set to 0 the velocity innovations will be clipped instead of
  // rejected if they exceed the gate size and a smaller value of EK3_VEL_I_GATE not exceeding 300
  ...
  // @Description: This sets the GPS horizontal position observation noise. Increasing it reduces
  ...
  // @Description: This sets the percentage number of standard deviations applied to the GPS
  // position measurement innovation consistency check. Decreasing it makes it more likely that good
  // measurements will be rejected. Increasing it makes it more likely that bad measurements will be
  // accepted. If EK3_GLITCH_RAD has been set to 0 the horizontal position innovations will be
  // clipped instead of rejected if they exceed the gate size so a smaller value of EK3_POS_I_GATE
  // not exceeding 300 is recommended to limit the effect of GPS transient errors.
  // @Range: 100 1000
  // @Increment: 25
  // @User: Advanced
  ...
  // @Description: This controls the maximum radial uncertainty in position between the value
  // predicted by the filter and the value measured by the GPS before the filter position and
  // velocity states are reset to the GPS. Making this value larger allows the filter to ignore
  // larger GPS glitches but also means that non-GPS errors such as IMU and compass can create a
  // larger error in position before the filter is forced back to the GPS position. If
  // EK3_GLITCH_RAD set to 0 the GPS innovations will be clipped instead of rejected if they exceed
  // the gate size set by EK3_VEL_I_GATE and EK3_POS_I_GATE which can be useful if poor quality
  // sensor data is causing GPS rejection and loss of navigation but does make the EKF more
  // susceptible to GPS glitches. If setting EK3_GLITCH_RAD to 0 it is recommended to reduce
  ...
  // @Description: This is the RMS value of noise in the altitude measurement. Increasing it reduces
  // the weighting of the baro measurement and will make the filter respond more slowly to baro
  // measurement errors, but will make it more sensitive to GPS and accelerometer errors. A larger
  // value for EK3_ALT_M_NSE may be required when operating with EK3_SRCx_POSZ = 0. This parameter
  // also sets the noise for the 'synthetic' zero height measurement that is used when EK3_SRCx_POSZ
  // = 0.
  // @Range: 0.1 100.0
  // @Increment: 0.1
  // @User: Advanced
  ...
  // @Description: This sets the percentage number of standard deviations applied to the height
  // measurement innovation consistency check. Decreasing it makes it more likely that good
  // measurements will be rejected. Increasing it makes it more likely that bad measurements will be
  // accepted.  If EK3_GLITCH_RAD set to 0 the vertical position innovations will be clipped instead
  // of rejected if they exceed the gate size and a smaller value of EK3_HGT_I_GATE not exceeding
  ...
  // @Description: This is the number of msec that the Height measurements lag behind the inertial
  // measurements.
  // @Range: 0 250
  // @Increment: 10
  // @User: Advanced
  ...
  // @Description: This is the RMS value of noise in magnetometer measurements. Increasing it
  ...
  // @Description: This determines when the filter will use the 3-axis magnetometer fusion model
  // that estimates both earth and body fixed magnetic field states and when it will use a simpler
  // magnetic heading fusion model that does not use magnetic field states. The 3-axis magnetometer
  // fusion is only suitable for use when the external magnetic field environment is stable.
  // EK3_MAG_CAL = 0 uses heading fusion on ground, 3-axis fusion in-flight, and is the default
  // setting for Plane users. EK3_MAG_CAL = 1 uses 3-axis fusion only when manoeuvring. EK3_MAG_CAL
  // = 2 uses heading fusion at all times, is recommended if the external magnetic field is varying
  // and is the default for rovers. EK3_MAG_CAL = 3 uses heading fusion on the ground and 3-axis
  // fusion after the first in-air field and yaw reset has completed, and is the default for
  // copters. EK3_MAG_CAL = 4 uses 3-axis fusion at all times. EK3_MAG_CAL = 5 uses an external yaw
  // sensor with simple heading fusion. NOTE : Use of simple heading magnetometer fusion makes
  // vehicle compass calibration and alignment errors harder for the EKF to detect which reduces the
  // sensitivity of the Copter EKF failsafe algorithm. NOTE: The fusion mode can be forced to 2 for
  // specific EKF cores using the EK3_MAG_MASK parameter. EK3_MAG_CAL = 6 uses an external yaw
  // sensor with fallback to compass when the external sensor is not available if we are flying.
  // NOTE: The fusion mode can be forced to 2 for specific EKF cores using the EK3_MAG_MASK
  // parameter. NOTE: limited operation without a magnetometer or any other yaw sensor is possible
  // by setting all COMPASS_USE, COMPASS_USE2, COMPASS_USE3, etc parameters to 0 and setting
  // COMPASS_ENABLE to 0. If this is done, the EK3_GSF_RUN and EK3_GSF_USE masks must be set to the
  // same as EK3_IMU_MASK. A yaw angle derived from IMU and GPS velocity data using a Gaussian Sum
  // Filter (GSF) will then be used to align the yaw when flight commences and there is sufficient
  // movement.
  // @Values: 0:When flying,1:When manoeuvring,2:Never,3:After first climb yaw reset,4:Always,5:Use
  // external yaw sensor (Deprecated in 4.1+ see EK3_SRCn_YAW),6:External yaw sensor with compass
  // fallback (Deprecated in 4.1+ see EK3_SRCn_YAW)
  // @User: Advanced
  // @RebootRequired: True
  ...
  // @Description: This sets the percentage number of standard deviations applied to the
  // magnetometer measurement innovation consistency check. Decreasing it makes it more likely that
  // good measurements will be rejected. Increasing it makes it more likely that bad measurements
  // will be accepted.
  // @Range: 100 1000
  // @Increment: 25
  // @User: Advanced
  ...
  // @Description: This is the RMS value of noise in equivalent airspeed measurements used by
  // planes. Increasing it reduces the weighting of airspeed measurements and will make wind speed
  // estimates less noisy and slower to converge. Increasing also increases navigation errors when
  // dead-reckoning without GPS measurements.
  // @Range: 0.5 5.0
  // @Increment: 0.1
  // @User: Advanced
  // @Units: m/s
  ...
  // @Description: This sets the percentage number of standard deviations applied to the airspeed
  // measurement innovation consistency check. Decreasing it makes it more likely that good
  // measurements will be rejected. Increasing it makes it more likely that bad measurements will be
  // accepted.
  // @Range: 100 1000
  ...
  // @Description: This is the RMS value of noise in the range finder measurement. Increasing it
  ...
  // @Description: This sets the percentage number of standard deviations applied to the range
  // finder innovation consistency check. Decreasing it makes it more likely that good measurements
  // will be rejected. Increasing it makes it more likely that bad measurements will be accepted.
  // @Range: 100 1000
  // @Increment: 25
  // @User: Advanced
  ...
  // @Description: The maximum optical flow rate in rad/sec that will be accepted by the filter.
  // Flow rates above this value will not be fused.
  // @Range: 1.0 4.0
  // @Increment: 0.1
  // @User: Advanced
  // @Units: rad/s
  ...
  // @Description: This is the RMS value of noise and errors in optical flow measurements.
  ...
  // @Description: This sets the percentage number of standard deviations applied to the optical
  // flow innovation consistency check. Decreasing it makes it more likely that good measurements
  // will be rejected. Increasing it makes it more likely that bad measurements will be accepted.
  // @Range: 100 1000
  // @Increment: 25
  // @User: Advanced
  ...
  // @Description: This is the number of msec that the optical flow measurements lag behind the
  // inertial measurements. It is the time from the end of the optical flow averaging period and
  // does not include the time delay due to the 100msec of averaging within the flow sensor.
  // @Range: 0 250
  // @Increment: 10
  // @User: Advanced
  // @Units: ms
  ...
  // @Description: This control disturbance noise controls the growth of estimated error due to gyro
  // measurement errors excluding bias. Increasing it makes the flter trust the gyro measurements
  // less and other measurements more.
  // @Range: 0.0001 0.1
  // @Increment: 0.0001
  // @User: Advanced
  // @Units: rad/s
  ...
  // @Description: This control disturbance noise controls the growth of estimated error due to
  // accelerometer measurement errors excluding bias. Increasing it makes the flter trust the
  // accelerometer measurements less and other measurements more.
  // @Range: 0.01 1.0
  // @Increment: 0.01
  // @User: Advanced
  // @Units: m/s/s
  ...
  // @Description: This state  process noise controls growth of the gyro delta angle bias state
  // error estimate. Increasing it makes rate gyro bias estimation faster and noisier.
  // @Range: 0.00001 0.001
  // @User: Advanced
  // @Units: rad/s/s
  ...
  // @Description: This noise controls the growth of the vertical accelerometer delta velocity bias
  // state error estimate. Increasing it makes accelerometer bias estimation faster and noisier.
  // @Range: 0.00001 0.02
  // @User: Advanced
  // @Units: m/s/s/s
  AP_GROUPINFO("ABIAS_P_NSE", 28, NavEKF3, _accelBiasProcessNoise, ABIAS_P_NSE_DEFAULT),

  // 29 previously used for EK2_MAG_P_NSE parameter that has been replaced with EK3_MAGE_P_NSE and
  // EK3_MAGB_P_NSE

  // @Param: WIND_P_NSE
  // @DisplayName: Wind velocity process noise (m/s^2)
  // @Description: This state process noise controls the growth of wind state error estimates.
  ...
  // @Description: This controls how much the process noise on the wind states is increased when
  // gaining or losing altitude to take into account changes in wind speed and direction with
  // altitude. Increasing this parameter increases how rapidly the wind states adapt when changing
  // altitude, but does make wind velocity estimation noiser.
  // @Range: 0.0 2.0
  // @Increment: 0.1
  // @User: Advanced
  ...
  // @Description: This is a 1 byte bitmap controlling which GPS preflight checks are performed. Set
  // to 0 to bypass all checks. Set to 255 perform all checks. Set to 3 to check just the number of
  // satellites and HDoP. Set to 31 for the most rigorous checks that will still allow checks to
  // pass when the copter is moving, eg launch from a boat.
  // @Bitmask: 0:NSats,1:HDoP,2:speed error,3:position error,4:yaw error,5:pos drift,6:vert
  // speed,7:horiz speed
  // @User: Advanced
  ...
  // @Description: 1 byte bitmap of IMUs to use in EKF3. A separate instance of EKF3 will be started
  // for each IMU selected. Set to 1 to use the first IMU only (default), set to 2 to use the second
  // IMU only, set to 3 to use the first and second IMU. Additional IMU's can be used up to a
  // maximum of 6 if memory and processing resources permit. There may be insufficient memory and
  // processing resources to run multiple instances. If this occurs EKF3 will fail to start.
  ...
  // @Description: This scales the thresholds that are used to check GPS accuracy before it is used
  // by the EKF. A value of 100 is the default. Values greater than 100 increase and values less
  // than 100 reduce the maximum GPS error the EKF will accept. A value of 200 will double the
  // allowable GPS error.
  // @Range: 50 200
  // @User: Advanced
  // @Units: %
  ...
  // @Description: This sets the amount of position variation that the EKF allows for when operating
  // without external measurements (eg GPS or optical flow). Increasing this parameter makes the EKF
  // attitude estimate less sensitive to vehicle manoeuvres but more sensitive to IMU errors.
  // @Range: 0.5 50.0
  // @User: Advanced
  // @Units: m
  ...
  // @Description: 1 byte bitmap controlling use of sideslip angle fusion for estimation of non wind
  // states during operation of 'fly forward' vehicle types such as fixed wing planes. By assuming
  // that the angle of sideslip is small, the wind velocity state estimates are corrected  whenever
  // the EKF is not dead reckoning (e.g. has an independent velocity or position sensor such as
  // GPS). This behaviour is on by default and cannot be disabled. When the EKF is dead reckoning,
  // the wind states are used as a reference, enabling use of the small angle of sideslip assumption
  // to correct non wind velocity states (eg attitude, velocity, position, etc) and improve
  // navigation accuracy. This behaviour is on by default and cannot be disabled. The behaviour
  // controlled by this parameter is the use of the small angle of sideslip assumption to correct
  // non wind velocity states when the EKF is NOT dead reckoning. This is primarily of benefit to
  // reduce the buildup of yaw angle errors during straight and level flight without a yaw sensor
  // (e.g. magnetometer or dual antenna GPS yaw) provided aerobatic flight maneuvers with large
  // sideslip angles are not performed. The 'always' option might be used where the yaw sensor is
  // intentionally not fitted or disabled. The 'WhenNoYawSensor' option might be used if a yaw
  // sensor is fitted, but protection against in-flight failure and continual rejection by the EKF
  // is desired. For vehicles operated within visual range of the operator performing frequent
  ...
  // @Description: This is the RMS value of noise in yaw measurements from the magnetometer.
  // Increasing it reduces the weighting on these measurements.
  // @Range: 0.05 1.0
  // @Increment: 0.05
  // @User: Advanced
  ...
  // @Description: This sets the percentage number of standard deviations applied to the
  // magnetometer yaw measurement innovation consistency check. Decreasing it makes it more likely
  // that good measurements will be rejected. Increasing it makes it more likely that bad
  ...
  // @Description: Sets the time constant of the output complementary filter/predictor in
  // centi-seconds.
  // @Range: 10 50
  // @Increment: 5
  // @User: Advanced
  ...
  // @Description: This state process noise controls the growth of earth magnetic field state error
  // estimates. Increasing it makes earth magnetic field estimation faster and noisier.
  // @Range: 0.00001 0.01
  // @User: Advanced
  // @Units: Gauss/s
  ...
  // @Description: This state process noise controls the growth of body magnetic field state error
  // estimates. Increasing it makes magnetometer bias error estimation faster and noisier.
  // @Range: 0.00001 0.01
  // @User: Advanced
  // @Units: Gauss/s
  ...
  // of its maximum range (see RNGFNDx_MAX) and the primary height source is Baro or GPS (see
  // EK3_SRCx_POSZ).  This feature should not be used for terrain following as it is designed for
  // vertical takeoff and landing with climb above the range finder use height before commencing the
  // mission, and with horizontal position changes below that height being limited to a flat region
  // around the takeoff and landing point.
  // @Range: -1 70
  // @Increment: 1
  // @User: Advanced
  // @Units: %
  ...
  // @Description: Specifies the maximum gradient of the terrain below the vehicle when it is using
  ...
  // @Description: This is the RMS value of noise in the range beacon measurement. Increasing it
  // reduces the weighting on this measurement.
  // @Range: 0.1 10.0
  // @Increment: 0.1
  // @User: Advanced
  ...
  // @Description: This sets the percentage number of standard deviations applied to the range
  // beacon measurement innovation consistency check. Decreasing it makes it more likely that good
  // measurements will be rejected. Increasing it makes it more likely that bad measurements will be
  // accepted.
  // @Range: 100 1000
  // @Increment: 25
  ...
  // @Description: This is the number of msec that the range beacon measurements lag behind the
  ...
  // @Description: The range finder will not be used as the primary height source when the
  ...
  // @Description: 1 byte bitmap of EKF cores that will disable magnetic field states and use simple
  // magnetic heading fusion at all times. This parameter enables specified cores to be used as a
  // backup for flight into an environment with high levels of external magnetic interference which
  // may degrade the EKF attitude estimate when using 3-axis magnetometer fusion. NOTE : Use of a
  // different magnetometer fusion algorithm on different cores makes unwanted EKF core switches due
  // to magnetometer errors more likely.
  ...
  // @Description: When a height sensor other than GPS is used as the primary height source by the
  // EKF, the position of the zero height datum is defined by that sensor and its frame of
  // reference. If a GPS height measurement is also available, then the height of the WGS-84 height
  // datum used by the EKF can be corrected so that the height returned by the getLLH() function is
  // compensated for primary height sensor drift and change in datum over time. The first two bit
  // positions control when the height datum will be corrected. Correction is performed using a
  // Bayes filter and only operates when GPS quality permits. The third bit position controls where
  // the corrections to the GPS reference datum are applied. Corrections can be applied to the local
  // vertical position or to the reported EKF origin height (default).
  // @Bitmask: 0:Correct when using Baro height,1:Correct when using range finder height,2:Apply
  // corrections to local position
  // @User: Advanced
  // @RebootRequired: True
  ...
  // @Description: This is the 1-STD odometry velocity observation error that will be assumed when
  // maximum quality is reported by the sensor. When quality is between max and min, the error will
  // be calculated using linear interpolation between VIS_VERR_MIN and VIS_VERR_MAX.
  // @Range: 0.05 0.5
  // @Increment: 0.05
  // @User: Advanced
  // @Units: m/s
  ...
  // @Description: This is the 1-STD odometry velocity observation error that will be assumed when
  // minimum quality is reported by the sensor. When quality is between max and min, the error will
  // be calculated using linear interpolation between VIS_VERR_MIN and VIS_VERR_MAX.
  // @Range: 0.5 5.0
  // @Increment: 0.1
  // @User: Advanced
  ...
  // @Description: Controls if the optical flow data is fused into the 24-state navigation estimator
  // OR the 1-state terrain height estimator.
  // @User: Advanced
  ...
  // @Description: Specifies the crossover frequency of the complementary filter used to calculate
  ...
  // @Description: This limits the difference between the learned earth magnetic field and the earth
  // field from the world magnetic model tables. A value of zero means to disable the use of the WMM
  // tables.
  // @User: Advanced
  // @Range: 0 500
  // @Units: mGauss
  ...
  // @Description: 1 byte bitmap of which EKF3 instances run an independent EKF-GSF yaw estimator to
  // provide a backup yaw estimate that doesn't rely on magnetometer data. This estimator uses IMU,
  // GPS and, if available, airspeed data. EKF-GSF yaw estimator data for the primary EKF3 instance
  // will be logged as GSF0 and GSF1 messages. Use of the yaw estimate generated by this algorithm
  // is controlled by the EK3_GSF_USE_MASK and EK3_GSF_RST_MAX parameters. To run the EKF-GSF yaw
  ...
  // @Description: A bitmask of which EKF3 instances will use the output from the EKF-GSF yaw
  // estimator that has been turned on by the EK3_GSF_RUN_MASK parameter. If the inertial navigation
  // calculation stops following the GPS, then the vehicle code can request EKF3 to attempt to
  // resolve the issue, either by performing a yaw reset if enabled by this parameter by switching
  // to another EKF3 instance.
  // @Bitmask: 0:FirstEKF,1:SecondEKF,2:ThirdEKF,3:FourthEKF,4:FifthEKF,5:SixthEKF
  // @User: Advanced
  // @RebootRequired: True
  ...
  // @Description: Sets the maximum number of times the EKF3 will be allowed to reset its yaw to the
  // estimate from the EKF-GSF yaw estimator. No resets will be allowed unless the use of the
  // EKF-GSF yaw estimate is enabled via the EK3_GSF_USE_MASK parameter.
  // @Range: 1 10
  // @Increment: 1
  // @User: Advanced
  // @RebootRequired: True
  ...
  // @Description: lanes have to be consistently better than the primary by at least this threshold
  // to reduce their overall relativeCoreError, lowering this makes lane switching more sensitive to
  // smaller error differences
  // @Range: 0.05 1
  // @Increment: 0.05
  // @User: Advanced
  ...
  // @User: Advanced
  // @Bitmask:
  // 0:EnableGPSAffinity,1:EnableBaroAffinity,2:EnableCompassAffinity,3:EnableAirspeedAffinity
  // @RebootRequired: True
...
const AP_Param::GroupInfo NavEKF3::var_info2[] = {
  ...
  // @Description: Ratio of mass to drag coefficient measured along the X body axis. This parameter
  // enables estimation of wind drift for vehicles with bluff bodies and without propulsion forces
  // in the X and Y direction (eg multicopters). The drag produced by this effect scales with speed
  // squared. Set to a positive value > 1.0 to enable. A starting value is the mass in Kg divided by
  // the frontal area. The predicted drag from the rotors is specified separately by the
  // EK3_DRAG_MCOEF parameter.
  // @Range: 0.0 1000.0
  // @Units: kg/m/m
  // @User: Advanced
  ...
  // @Description: Ratio of mass to drag coefficient measured along the Y body axis. This parameter
  // enables estimation of wind drift for vehicles with bluff bodies and without propulsion forces
  // in the X and Y direction (eg multicopters). The drag produced by this effect scales with speed
  // squared. Set to a positive value > 1.0 to enable. A starting value is the mass in Kg divided by
  // the side area. The predicted drag from the rotors is specified separately by the EK3_DRAG_MCOEF
  // parameter.
  // @Range: 50.0 1000.0
  // @Units: kg/m/m
  // @User: Advanced
  ...
  // @Description: This sets the amount of noise used when fusing X and Y acceleration as an
  // observation that enables estimation of wind velocity for multi-rotor vehicles. This feature is
  // enabled by the EK3_DRAG_BCOEF_X and EK3_DRAG_BCOEF_Y parameters
  // @Range: 0.1 2.0
  // @Increment: 0.1
  // @User: Advanced
  // @Units: m/s/s
  ...
  // @Description: This parameter is used to predict the drag produced by the rotors when flying a
  // multi-copter, enabling estimation of wind drift. The drag produced by this effect scales with
  // speed not speed squared and is produced because some of the air velocity normal to the rotors
  // axis of rotation is lost when passing through the rotor disc which changes the momentum of the
  // airflow causing drag. For unducted rotors the effect is roughly proportional to the area of the
  // propeller blades when viewed side on and changes with different propellers. It is higher for
  // ducted rotors. For example if flying at 15 m/s at sea level conditions produces a rotor induced
  // drag acceleration of 1.5 m/s/s, then EK3_DRAG_MCOEF would be set to 0.1 = (1.5/15.0). Set
  // EK3_MCOEF to a positive value to enable wind estimation using this drag effect. To account for
  // the drag produced by the body which scales with speed squared, see documentation for the
  ...
  // @Description: This parameter is adjust the sensitivity of the on ground not moving test which
  // is used to assist with learning the yaw gyro bias and stopping yaw drift before flight when
  // operating without a yaw sensor. Bigger values allow the detection of a not moving condition
  // with noiser IMU data. Check the XKFM data logged when the vehicle is on ground not moving and
  // adjust the value of OGNM_TEST_SF to be slightly higher than the maximum value of the XKFM.ADR,
  // XKFM.ALR, XKFM.GDR and XKFM.GLR test levels.
  // @Range: 1.0 10.0
  // @Increment: 0.5
  // @User: Advanced
  ...
  // @Description: This parameter sets the size of the dead zone that is applied to negative baro
  // height spikes that can occur when taking off or landing when a vehicle with lift rotors is
  // operating in ground effect ground effect. Set to about 0.5m less than the amount of negative
  // offset in baro height that occurs just prior to takeoff when lift motors are spooling up. Set
  // to 0 if no ground effect is present.
  // @Range: 0.0 10.0
  // @Increment: 0.5
  // @User: Advanced
  ...
  // @Description: The core number (index in IMU mask) that will be used as the primary EKF core on
  // startup. While disarmed the EKF will force the use of this core. A value of 0 corresponds to
  // the first IMU in EK3_IMU_MASK.
  // @Range: 0 2
  // @Increment: 1
  // @User: Advanced
  ...
  // @Description: Determines how verbose the EKF3 streaming logging is. A value of 0 provides full
  // logging(default), a value of 1 only XKF4 scaled innovations are logged, a value of 2 both XKF4
  // and GSF are logged, and a value of 3 disables all streaming logging of EKF3.
  // @Range: 0 3
  // @Increment: 1
  // @User: Advanced
  ...
  // @Description: Vertical accuracy threshold for GPS as the altitude source. The GPS will not be
  // used as an altitude source if the reported vertical accuracy of the GPS is larger than this
  // threshold, falling back to baro instead. Set to zero to deactivate the threshold check.
  // @Range: 0.0 10.0
  // @Increment: 0.1
  // @User: Advanced
  // @Units: m
  ...
  // @Description: EKF optional behaviour. Bit 0 (JammingExpected): Setting JammingExpected will
  // change the EKF behaviour such that if dead reckoning navigation is possible it will require the
  // preflight alignment GPS quality checks controlled by EK3_GPS_CHECK and EK3_CHECK_SCALE to pass
  // before resuming GPS use if GPS lock is lost for more than 2 seconds to prevent bad position
  // estimate. Bit 1 (Manual lane switching): DANGEROUS – If enabled, this disables automatic lane
  // switching. If the active lane becomes unhealthy, no automatic switching will occur. Users must
  // manually set EK3_PRIMARY to change lanes. No health checks will be performed on the selected
  // lane. Use with extreme caution.  Bit 2 (Optflow may use terrain alt): Terrain SRTM data will be
  // used if the vehicle climbs above the rangefinder's range allowing optical flow to be used at
  // higher altitudes.
...
bool NavEKF3::InitialiseFilter(void)
{
  ...
  if (core == nullptr) {
    ...
    core =
      (NavEKF3_core *)dal.malloc_type(sizeof(NavEKF3_core) * num_cores, AP_DAL::MemoryType::FAST);
    if (core == nullptr) {
      _enable.set(0);
      num_cores = 0;
  ...
  for (uint8_t core_index = 0; core_index < num_cores; core_index++) {
    if (coreSetupRequired[core_index]) {
      coreSetupRequired[core_index] =
        !core[core_index].setup_core(coreImuIndex[core_index], core_index);
      if (coreSetupRequired[core_index]) {
...
void NavEKF3::UpdateFilter(void)
{
  ...
  for (uint8_t i = 0; i < num_cores; i++) {
    ...
    if (
      core[i].getFramesSincePredict() < (_framesPerPrediction + 3) &&
  ...
  // If the current core selected has a bad error score or is unhealthy, switch to a healthy core
  // with the lowest fault score Don't start running the check until the primary core has started
  // returned healthy for at least 10 seconds to avoid switching due to initial alignment
  ...
  // core selection is only available after the vehicle is armed, else forced to lane 0 if its
  // healthy
  if (runCoreSelection && armed && lane_switching_enabled) {
    // update this instance's error scores for all active cores and get the primary core's error
    // score
    ...
    for (uint8_t coreIndex = 0; coreIndex < num_cores; coreIndex++) {
      if (coreIndex != primary) {
        ...
        if (altCoreAvailable) {
          // if this core has a significantly lower relative error to the active primary, we
          // consider it as a better core and would like to switch to it even if the current primary
          // is healthy
          betterCore =
            altCoreError <= -BETTER_THRESH;  // a better core if its relative error is below a
                                             // substantial level than the primary's
          // handle the case where the secondary core is faster to complete yaw alignment which can
          // happen in flight when oeprating without a magnetomer
    ...
    // Switch cores if another core is available and the active primary core meets one of the
...
void NavEKF3::checkLaneSwitch(void)
{
  ...
  for (uint8_t coreIndex = 0; coreIndex < num_cores; coreIndex++) {
    if (coreIndex != primary) {
      const NavEKF3_core & newCore = core[coreIndex];
      // an alternative core is available for selection only if healthy and if states have been
      // updated on this time step
      bool altCoreAvailable =
        newCore.healthy() && newCore.have_aligned_yaw() && newCore.have_aligned_tilt();
...
void NavEKF3::updateCoreRelativeErrors()
{
  ...
  for (uint8_t i = 0; i < num_cores; i++) {
    if (i != primary) {
      ...
      // reduce error for a core only if its better than the primary lane by at least the Relative
      // Error Threshold, this should prevent unnecessary lane changes
...
// returns false if we fail arming checks, in which case the buffer will be populated with a failure
// message requires_position should be true if horizontal position configuration should be checked
bool NavEKF3::pre_arm_check(
  bool requires_position, char * failure_msg, uint8_t failure_msg_len) const
{
  ...
  // check if using compass (i.e. EK3_SRCn_YAW) with deprecated MAG_CAL values (5 was EXTERNAL_YAW,
  // 6 was EXTERNAL_YAW_FALLBACK)
  const int8_t magCalParamVal = _magCal.get();
  for (uint8_t i = 0; i < num_cores; i++) {
    const AP_NavEKF_Source::SourceYaw yaw_source = sources.getYawSource(i);
    if (
      ((magCalParamVal == 5) || (magCalParamVal == 6)) &&
...
// return the innovation in m/s, innovation variance in (m/s)^2 and age in msec of the last TAS
// measurement processed for a given sensor instance
bool NavEKF3::getAirSpdHealthData(
  uint8_t instance, float & innovation, float & innovationVariance, uint32_t & age_ms) const
{
  if (core == nullptr) {
    return false;
  }
...
{
  // return the value calculated from a complementary filter applied to the EKF height and vertical
  // acceleration
  if (core) {
...
// return the scale factor to be applied to navigation velocity gains to compensate for increase in
// velocity noise with height when using optical flow
...
bool NavEKF3::getMagOffsets(uint8_t mag_idx, Vector3f & magOffsets) const
{
  ...
  // try the primary first, else loop through all of the cores and return when one has offsets for
  // this mag instance
...
// The getFilterStatus() function provides a more detailed description of data health and must be
// checked if data is to be used for flight control
...
bool NavEKF3::getInnovations(
  Vector3f & velInnov, Vector3f & posInnov, Vector3f & magInnov, float & tasInnov,
  float & yawInnov) const
{
  if (core == nullptr) {
    return false;
  }
...
// return the innovation consistency test ratios for the velocity, position, magnetometer and true
// airspeed measurements
bool NavEKF3::getVariances(
  float & velVar, float & posVar, float & hgtVar, Vector3f & magVar, float & tasVar,
...
bool NavEKF3::getVelInnovationsAndVariancesForSource(
  AP_NavEKF_Source::SourceXY source, Vector3f & innovations, Vector3f & variances) const
{
  if (core) {
...
{
  if (
    configuredToUseGPSForPosXY() ||
    sources.getPosZSource(primary) == AP_NavEKF_Source::SourceZ::GPS) {
    return true;
  }
  return false;
}
...
// The sign convention is that a RH physical rotation of the sensor about an axis produces both a
// positive flow and gyro rate msecFlowMeas is the scheduler time in msec when the optical flow data
// was received from the sensor. posOffset is the XYZ flow sensor position in the body frame in m
// heightOverride is the fixed height of the sensor above ground in m, when on rover vehicles. 0 if
// not used
#if EK3_FEATURE_OPTFLOW_FUSION
void NavEKF3::writeOptFlowMeas(
  const uint8_t rawFlowQuality, const Vector2f & rawFlowRates, const Vector2f & rawGyroRates,
  const uint32_t msecFlowMeas, const Vector3f & posOffset, float heightOverride)
{
  dal.writeOptFlowMeas(
    rawFlowQuality, rawFlowRates, rawGyroRates, msecFlowMeas, posOffset, heightOverride);

  if (core) {
    for (uint8_t i = 0; i < num_cores; i++) {
      core[i].writeOptFlowMeas(
        rawFlowQuality, rawFlowRates, rawGyroRates, msecFlowMeas, posOffset, heightOverride);
    }
  }
}
...
bool NavEKF3::getOptFlowSample(
  uint32_t & timeStamp_ms, Vector2f & flowRate, Vector2f & bodyRate, Vector2f & losPred) const
{
...
void NavEKF3::writeEulerYawAngle(
 ...
 *
 * pos        : XYZ position (m) in a RH navigation frame with the Z axis pointing down and XY axes
 * horizontal. Frame must be aligned with NED if the magnetomer is being used for yaw. quat       :
 * quaternion describing the rotation from navigation frame to body frame posErr     : 1-sigma
 * spherical position error (m) angErr     : 1-sigma spherical angle error (rad) timeStamp_ms :
 * system time the measurement was taken, not the time it was received (mSec) delay_ms   : average
 * delay of external nav system measurements relative to inertial measurements resetTime_ms : system
 * time of the last position reset request (mSec)
 *
 */
void NavEKF3::writeExtNavData(
  const Vector3f & pos, const Quaternion & quat, float posErr, float angErr, uint32_t timeStamp_ms,
  uint16_t delay_ms, uint32_t resetTime_ms)
{
 ...
 */
void NavEKF3::writeExtNavVelData(
  const Vector3f & vel, float err, uint32_t timeStamp_ms, uint16_t delay_ms)
{
 ...
 * delPos is the XYZ change in linear position measured in body frame and relative to the inertial
 * reference at timeStamp_ms (m) delAng is the XYZ angular rotation measured in body frame and
 * relative to the inertial reference at timeStamp_ms (rad) delTime is the time interval for the
 * measurement of delPos and delAng (sec) timeStamp_ms is the timestamp of the last image used to
 * calculate delPos and delAng (msec) delay_ms is the average delay of external nav system
 * measurements relative to inertial measurements posOffset is the XYZ body frame position of the
 * camera focal point (m)
 */
void NavEKF3::writeBodyFrameOdom(
  float quality, const Vector3f & delPos, const Vector3f & delAng, float delTime,
  uint32_t timeStamp_ms, uint16_t delay_ms, const Vector3f & posOffset)
{
  dal.writeBodyFrameOdom(quality, delPos, delAng, delTime, timeStamp_ms, delay_ms, posOffset);

  if (core) {
    for (uint8_t i = 0; i < num_cores; i++) {
      core[i].writeBodyFrameOdom(
        quality, delPos, delAng, delTime, timeStamp_ms, delay_ms, posOffset);
    }
  }
}

/*
 * Write odometry data from a wheel encoder. The axis of rotation is assumed to be parallel to the
 * vehicle body axis
 *
 * delAng is the measured change in angular position from the previous measurement where a positive
 * rotation is produced by forward motion of the vehicle (rad) delTime is the time interval for the
 * measurement of delAng (sec) timeStamp_ms is the time when the rotation was last measured (msec)
 * posOffset is the XYZ body frame position of the wheel hub (m)
 */
void NavEKF3::writeWheelOdom(
  float delAng, float delTime, uint32_t timeStamp_ms, const Vector3f & posOffset, float radius)
{
...
void NavEKF3::convert_parameters()
{
  ...
  if (found_gps_type) {
    switch (gps_type_old.get()) {
      case 0:
        ...
        break;
      case 1:
        // EK3_GPS_TYPE == 1 (GPS 2D Vel and 2D Pos) then EK3_SRC1_POSXY = GPS(1), EK3_SRC1_VELXY =
        // GPS(1), EK3_SRC1_VELZ = NONE(0)
        AP_Param::set_and_save_by_name("EK3_SRC1_POSXY", (int8_t)AP_NavEKF_Source::SourceXY::GPS);
        AP_Param::set_and_save_by_name("EK3_SRC1_VELXY", (int8_t)AP_NavEKF_Source::SourceXY::GPS);
        AP_Param::set_and_save_by_name("EK3_SRC1_VELZ", (int8_t)AP_NavEKF_Source::SourceZ::NONE);
        break;
      case 2:
        // EK3_GPS_TYPE == 2 (GPS 2D Pos) then EK3_SRC1_POSXY = GPS(1), EK3_SRC1_VELXY = None(0),
        // EK3_SRC1_VELZ = NONE(0)
        AP_Param::set_and_save_by_name("EK3_SRC1_POSXY", (int8_t)AP_NavEKF_Source::SourceXY::GPS);
      ...
      case 3:
      default:
        // EK3_GPS_TYPE == 3 (No GPS) we don't know what to do, could be optical flow, beacon or
        // external nav
  ...
  const AP_Param::ConversionInfo alt_source_info = {
    k_param_ekf3, 9, AP_PARAM_INT8, "EK3_ALT_SOURCE"};
  ...
  if (AP_Param::find_old_parameter(&alt_source_info, &alt_source_old)) {
    switch (alt_source_old.get()) {
      ...
      case 1:
        ...
        AP_Param::set_and_save_by_name(
          "EK3_SRC1_POSZ", (int8_t)AP_NavEKF_Source::SourceZ::RANGEFINDER);
        break;
      case 2:
  ...
  switch (_magCal.get()) {
    case 5:
      // EK3_MAG_CAL = 5 (External Yaw sensor).  We rely on effective_magCal to interpret old "5"
      // values as "Never"
      AP_Param::set_and_save_by_name("EK3_SRC1_YAW", (int8_t)AP_NavEKF_Source::SourceYaw::GPS);
      break;
    case 6:
      // EK3_MAG_CAL = 6 (ExtYaw with Compass fallback).  We rely on effective_magCal to interpret
      // old "6" values as "When Flying"
      AP_Param::set_and_save_by_name(
        "EK3_SRC1_YAW", (int8_t)AP_NavEKF_Source::SourceYaw::GPS_COMPASS_FALLBACK);
      break;
    default:
      // do nothing
      break;
...
// Returns the amount of yaw angle change (in radians) due to the last yaw angle reset or core
// selection switch Returns the time of the last yaw angle reset or 0 if no reset or core switch has
// ever occurred Where there are multiple consumers, they must access this function on the same
// frame as each other
...
// Where there are multiple consumers, they must access this function on the same frame as each
// other
...
void NavEKF3::updateLaneSwitchYawResetData(uint8_t new_primary, uint8_t old_primary)
{
  ...
  // Record the yaw delta between current core and new primary core and the timestamp of the core
  // change Add current delta in case it hasn't been consumed yet
  core[old_primary].getEulerAngles(eulers_old_primary);
  core[new_primary].getEulerAngles(eulers_new_primary);
  yaw_reset_data.core_delta =
    wrap_PI(eulers_new_primary.z - eulers_old_primary.z + yaw_reset_data.core_delta);
...
void NavEKF3::updateLaneSwitchPosResetData(uint8_t new_primary, uint8_t old_primary)
{
  ...
  if (
    core[old_primary].getLastPosNorthEastReset(old_pos_delta) > pos_reset_data.last_function_call) {
    pos_reset_data.core_delta += old_pos_delta;
  }

  // Record the position delta between current core and new primary core and the timestamp of the
  // core change Add current delta in case it hasn't been consumed yet
  core[old_primary].getPosNE(pos_old_primary);
  core[new_primary].getPosNE(pos_new_primary);
  pos_reset_data.core_delta =
    (pos_new_primary - pos_old_primary).tofloat() + pos_reset_data.core_delta;
...
void NavEKF3::updateLaneSwitchPosDownResetData(uint8_t new_primary, uint8_t old_primary)
{
  ...
  // If current primary has reset position after controller got it, add it to the delta
  if (
    core[old_primary].getLastPosDownReset(oldPosDownDelta) >
    pos_down_reset_data.last_function_call) {
  ...
  pos_down_reset_data.core_delta =
    posDownNewPrimary - posDownOldPrimary + pos_down_reset_data.core_delta;
  pos_down_reset_data.last_primary_change = imuSampleTime_us / 1000;
  pos_down_reset_data.core_changed = true;
}

// Writes the default equivalent airspeed and 1-sigma uncertainty in m/s to be used in forward
// flight if a measured airspeed is required and not available.
...
bool NavEKF3::getGyroVariance(float var[3]) const
{
  if (!core) {
    return false;
  }
  core[primary].getGyroVariance(var);
  return true;
}

bool NavEKF3::getAccelVariance(float var[3]) const
{
  if (!core) {
    return false;
  }
