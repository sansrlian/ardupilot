...
// Number of milliseconds of bad IMU data before a reset to vertical position and velocity height
// sources is performed
#define BAD_IMU_DATA_TIMEOUT_MS 1000

// number of milliseconds the bad IMU data response settings will be held after the last bad IMU
// data is detected
...
public:
  ...
  // measurement processed returns false if the data is unavailable
  bool getAirSpdHealthData(float & innovation, float & innovationVariance, uint32_t & age_ms) const;

  // Return the rate of change of vertical position in the down direction (dPosD/dt) in m/s
  // This can be different to the z component of the EKF velocity state because it will fluctuate
  // with height errors and corrections in the EKF but will always be kinematically consistent with
  // the z component of the EKF position state
  ...
  // return the scale factor to be applied to navigation velocity gains to compensate for increase
  // in velocity noise with height when using optical flow
  void getEkfControlLimits(float & ekfGndSpdLimit, float & ekfNavVelGainScaler) const;

  // return the NED wind speed estimates in m/s (positive is air moving in the direction of the
  // axis) returns true if wind state estimation is active
  ...
  // Set the EKF's NE horizontal position states and their corresponding variances from a supplied
  // WGS-84 location and uncertainty The altitude element of the location is not used. Returns true
  ...
  bool getVelInnovationsAndVariancesForSource(
    AP_NavEKF_Source::SourceXY source, Vector3f & innovations,
    Vector3f & variances) const WARN_IF_UNUSED;
  ...
  // positive flow and gyro rate msecFlowMeas is the scheduler time in msec when the optical flow
  // data was received from the sensor. posOffset is the XYZ flow sensor position in the body frame
  // in m heightOverride is the fixed height of the sensor above ground in m, when on rover
  // vehicles. 0 if not used
   ...
   * reference at time_ms (m) delAng is the XYZ angular rotation measured in body frame and relative
   * to the inertial reference at time_ms (rad) delTime is the time interval for the measurement of
   * delPos and delAng (sec) timeStamp_ms is the timestamp of the last image used to calculate
   * delPos and delAng (msec) delay_ms is the average delay of external nav system measurements
   * relative to inertial measurements posOffset is the XYZ body frame position of the camera focal
   * point (m)
   */
   ...
   * vehicle body axis
   *
   * delAng is the measured change in angular position from the previous measurement where a
   * positive rotation is produced by forward motion of the vehicle (rad) delTime is the time
   * interval for the measurement of delAng (sec) timeStamp_ms is the time when the rotation was
   * last measured (msec) posOffset is the XYZ body frame position of the wheel hub (m) radius is
   * the effective rolling radius of the wheel (m)
   */
  void writeWheelOdom(
   ...
   * timeStamp_ms: System time in msec when the yaw measurement was taken. This time stamp must
   * include all measurement lag and transmission delays. type: An integer specifying Euler rotation
   * order used to define the yaw angle. type = 1 specifies a 312 (ZXY) rotation order, type = 2
   * specifies a 321 (ZYX) rotation order.
   */
   ...
   *
   * pos        : position in the RH navigation frame. Frame is assumed to be NED if frameIsNED is
   * true. (m) quat       : quaternion desribing the rotation from navigation frame to body frame
   * posErr     : 1-sigma spherical position error (m)
   * angErr     : 1-sigma spherical angle error (rad)
   * timeStamp_ms : system time the measurement was taken, not the time it was received (mSec)
   * delay_ms   : average delay of external nav system measurements relative to inertial
   * measurements resetTime_ms : system time of the last position reset request (mSec)
   *
   */
  void writeExtNavData(
    const Vector3f & pos, const Quaternion & quat, float posErr, float angErr,
    uint32_t timeStamp_ms, uint16_t delay_ms, uint32_t resetTime_ms);
  ...
  */
  ...
  Return a filter function status that indicates:
      ...
      If the filter has activated the mode that mitigates against ground effect static pressure
  errors If GPS data is being used
  */
  ...
  void getGyroVariance(float var[3]) const;
  void getAccelVariance(float var[3]) const;
...
private:
  ...
  public:
    Vector9(
      ftype p0, ftype p1, ftype p2, ftype p3, ftype p4, ftype p5, ftype p6, ftype p7, ftype p8)
    {
      _v[0] = p0;
      _v[1] = p1;
      _v[2] = p2;
  ...
  {
  public:
    Vector5(ftype p0, ftype p1, ftype p2, ftype p3, ftype p4)
    {
      _v[0] = p0;
      _v[1] = p1;
      _v[2] = p2;
  ...
  struct state_elements
  {
    ...
    Vector2F wind_vel;        // horizontal North East wind velocity vector in local NED earth frame
                              // (m/sec) 22..23
  };
  ...
  {
    Vector2F flowRadXY;      // raw (non motion compensated) optical flow angular rates about the XY
                             // body axes (rad/sec)
    Vector2F flowRadXYcomp;  // motion compensated XY optical flow angular rates about the XY body
                             // axes (rad/sec)
    Vector3F bodyRadXYZ;     // body frame XYZ axis angular rates averaged across the optical flow
                             // measurement interval (rad/sec)
    Vector3F body_offset;    // XYZ position of the optical flow sensor in body frame (m)
    float heightOverride;    // The fixed height of the sensor above ground in m, when on rover
                             // vehicles. 0 if not used
  };
  ...
  {
    ftype delAng;  // wheel rotation angle measured in body frame - positive is forward movement of
                   // vehicle (rad/s)
  ...
  // Specifies the rotation order used for the Tait-Bryan or Euler angles where alternative rotation
  ...
  struct yaw_elements : EKF_obs_element_t
  {
    ...
    rotationOrder
      order;  // type specifiying Euler rotation order used, 0 = 321 (ZYX), 1 = 312 (ZXY)
  };
  ...
  // Checking the availability and quality of the data source specified is the responsibility of the
  // caller
  ...
  // Argument rotVarVecPtr is pointer to a vector defining the earth frame uncertainty variance of
  // the quaternion states used to perform a reset of the quaternion state covariances only. Set to
  // null for normal operation.
...
#if EK3_FEATURE_BEACON_FUSION
  // determine when to perform fusion of range measurements take relative to a beacon at a known NED
  // position
  ...
  // initialise the earth magnetic field states using declination and current attitude and
  // magnetometer measurements

  // align the yaw angle for the quaternion states to the given yaw angle which should be at the
  // fusion horizon
  ...
  // return true if the filter is ready to start using optical flow measurements for position and
  // velocity estimation
  ...
  // really_fuse should be true to actually fuse into the main filter, false to only calculate
  // variances
  ...
  // Determine if learning of wind and magnetic field will be enabled and set corresponding indexing
  // limits to avoid unnecessary operations
  ...
  // Fuse compass measurements using a direct yaw angle observation (doesn't require magnetic field
  // states) Returns true if the fusion was successful
  ...
  // Fuse declination angle to keep earth field declination from changing when we don't have earth
  // relative observations. Input is 1-sigma uncertainty in published declination
  ...
  void CalculateVelInnovationsAndVariances(
    const Vector3F & velocity, ftype noise, ftype accel_scale, Vector3F & innovations,
    Vector3F & variances) const;
  ...
  // reset the quaternion states using the supplied yaw angle, maintaining the previous roll and
  // pitch also reset the body to nav frame rotation matrix reset the quaternion state covariances
  // using the supplied yaw variance yaw          : new yaw angle (rad) yaw_variance : variance of
  // new yaw angle (rad^2) order : enum defining Tait-Bryan rotation order used in calculation of
  // the yaw angle
  ...
  bool velTimeout;  // boolean true if velocity measurements have failed innovation consistency
                    // check and timed out
  bool posTimeout;  // boolean true if position measurements have failed innovation consistency
                    // check and timed out
  bool hgtTimeout;  // boolean true if height measurements have failed innovation consistency check
                    // and timed out
  bool magTimeout;  // boolean true if magnetometer measurements have failed for too long and have
                    // timed out
  bool tasTimeout;  // boolean true if true airspeed measurements have failed for too long and have
                    // timed out
  bool
    dragTimeout;    // boolean true if drag measurements have failed for too long and have timed out
  bool badIMUdata;  // boolean true if the bad IMU data is detected
  bool velAiding;   // boolean true if the velocity drift is constrained by observations
  bool waitingForGpsChecks;  // boolean true if the EKF should write GPS data to the buffer until
  ...
  uint32_t vertVelVarClipCounter;  // counter used to control reset of vertical velocity variance
                                   // following collapse against the lower limit

  ftype gpsNoiseScaler;  // Used to scale the  GPS measurement noise and consistency gates to
                         // compensate for operation with small satellite counts
  ...
  bool manoeuvring;     // boolean true when the flight vehicle is performing horizontal changes in
                        // velocity
  Vector6 innovVelPos;  // innovation output for a group of measurements
  Vector6 varInnovVelPos;  // innovation variance output for a group of measurements
  Vector6 velPosObs;  // observations for combined velocity and positon group of measurements (3x1 m
                      // , 3x1 m/s)
  ...
  ftype defaultAirSpeed;  // default equivalent airspeed in m/s to be used if the measurement is
                          // unavailable. Do not use if not positive.
  ftype defaultAirSpeedVariance;  // default equivalent airspeed variance in (m/s)**2 to be used
                                  // when defaultAirSpeed is specified.
  bool magFusePerformed;  // boolean set to true when magnetometer fusion has been perfomred in that
                          // time step
  ...
  uint16_t hgtRetryTime_ms;      // time allowed without use of height measurements before a height
                                 // timeout is declared
  uint32_t lastVelPassTime_ms;   // time stamp when GPS velocity measurement last passed innovation
                                 // consistency check (msec)
  uint32_t lastGpsPosPassTime_ms;   // time stamp when GPS position measurement last passed
                                    // innovation consistency check (msec)
  uint32_t lastHgtPassTime_ms;      // time stamp when height measurement last passed innovation
                                    // consistency check (msec)
  uint32_t lastTasPassTime_ms;      // time stamp when airspeed measurement last passed innovation
                                    // consistency check (msec)
  uint32_t lastTasFailTime_ms;      // time stamp when airspeed measurement last failed innovation
                                    // consistency check (msec)
  uint32_t lastTimeGpsReceived_ms;  // last time we received GPS data
  uint32_t timeAtLastAuxEKF_ms;  // last time the auxiliary filter was run to fuse range or optical
                                 // flow measurements
  uint32_t lastHealthyMagTime_ms;  // time the magnetometer was last declared healthy
  bool allMagSensorsFailed;  // true if all magnetometer sensors have timed out on this flight and
                             // we are no longer using magnetometer data
  ...
  ftype posDownAtTakeoff;  // flight vehicle vertical position sampled at transition from on-ground
                           // to in-air and used as a reference (m)
  bool useGpsVertVel;      // true if GPS vertical velocity should be used
  ftype yawResetAngle;     // Change in yaw angle due to last in-flight yaw reset in radians. A
                           // positive value means the yaw angle has increased.
  uint32_t lastYawReset_ms;       // System time at which the last yaw reset occurred. Returned by
  ...
  uint8_t yawAlignGpsValidCount;  // number of continuous good GPS velocity samples used for in
                                  // flight yaw alignment
  ...
  imu_elements imuDataDownSampledNew;  // IMU data at the current time horizon that has been
                                       // downsampled to a 100Hz rate
  QuaternionF imuQuatDownSampleNew;  // Quaternion obtained by rotating through the IMU delta angles
                                     // since the start of the current down sampled frame
  ...
  Vector3F delAngCorrection;  // correction applied to delta angles used by output observer to track
                              // the EKF
  ...
  ftype badImuVelErrIntegral;   // integral of output predictor D velocity tracking error when bad
                                // IMU data is detected (m)
  ...
  uint32_t magYawResetTimer_ms;  // timer in msec used to track how long good magnetometer data is
                                 // failing innovation consistency checks
  ...
  bool airDataFusionWindOnly;  // true when  sideslip and airspeed fusion is only allowed to modify
                               // the wind states
  Vector3F lastMagOffsets;     // Last magnetometer offsets from COMPASS_ parameters. Used to detect
                               // parameter changes.
  ...
  Vector2F posResetNE;       // Change in North/East position due to last in-flight reset in metres.
                             // Returned by getLastPosNorthEastReset
  uint32_t lastPosReset_ms;  // System time at which the last position reset occurred. Returned by
                             // getLastPosNorthEastReset
  Vector2F velResetNE;  // Change in North/East velocity due to last in-flight reset in metres/sec.
                        // Returned by getLastVelNorthEastReset
  uint32_t lastVelReset_ms;  // System time at which the last velocity reset occurred. Returned by
                             // getLastVelNorthEastReset
  ftype posResetD;  // Change in Down position due to last in-flight reset in metres. Returned by
                    // getLastPosDowntReset
  uint32_t lastPosResetD_ms;  // System time at which the last position reset occurred. Returned by
                              // getLastPosDownReset
  ftype yawTestRatio;  // square of magnetometer yaw angle innovation divided by fail threshold
  QuaternionF prevQuatMagReset;  // Quaternion from the last time the magnetic field state reset
                                 // condition test was performed
  ftype hgtInnovFiltState;  // state used for fitering of the height innovations used for pre-flight
                            // checks
  ...
  ftype posDownObsNoise;  // observation noise variance on the vertical position used by the state
                          // and covariance update step (m^2)
  ...
  double ekfGpsRefHgt;  // floating point representation of the WGS-84 reference height used to
                        // convert GPS height to local height (m)
  uint32_t lastOriginHgtTime_ms;  // last time the ekf's WGS-84 origin height was corrected
  Vector3F outputTrackError;      // attitude (rad), velocity (m/s) and position (m) tracking error
                                  // magnitudes from the output observer
  Vector3F velOffsetNED;  // This adds to the earth frame velocity estimate at the IMU to give the
                          // velocity at the body origin (m/s)
  Vector3F posOffsetNED;  // This adds to the earth frame position estimate at the IMU to give the
                          // position at the body origin (m)
  uint32_t firstInitTime_ms;  // First time the initialise function was called (msec)
  uint32_t
    lastInitFailReport_ms;  // Last time the buffer initialisation failure report was sent (msec)
  ftype tiltErrorVariance;  // variance of the angular uncertainty measured perpendicular to the
                            // vertical (rad^2)

  // variables used to calculate a vertical velocity that is kinematically consistent with the
  // vertical position
  struct
  {
    ftype pos;
  ...
  uint32_t lastPreAlignGpsCheckTime_ms;  // last time in msec the GPS quality was checked during pre
                                         // alignment checks
  ftype gpsDriftNE;  // amount of drift detected in the GPS position during pre-flight GPs checks
  ftype gpsVertVelFilt;   // amount of filtered vertical GPS velocity detected during pre-flight GPS
                          // checks
  ftype gpsHorizVelFilt;  // amount of filtered horizontal GPS velocity detected during pre-flight
                          // GPS checks
  ...
  bool
    gpsAccuracyGood;  // true when the GPS accuracy is considered to be good enough for safe flight.
  bool gpsAccuracyGoodForAltitude;  // true when the GPS accuracy is considered to be good enough to
                                    // use it as an altitude source.
  Vector3F gpsVelInnov;             // gps velocity innovations
  Vector3F gpsVelVarInnov;          // gps velocity innovation variances
  uint32_t gpsRetrieveTime_ms;      // system time that GPS data was retrieved from the buffer (to
                                    // detect timeouts)
  ...
  uint32_t flowInnovTime_ms;     // system time that optical flow innovations and variances were
                                 // recorded (to detect timeouts)
#if EK3_FEATURE_OPTFLOW_FUSION
  ...
  {
    uint32_t
      timestamp_ms;  // system timestamp of last correct optical flow sample (used for calibration)
    ...
    Vector2f losPred;   // EKF estimated component of flowRate that comes from vehicle movement (not
                        // rotation)
  } flowCalSample;
  ...
  uint32_t
    terrain_srtm_alt_ms;  // system time when the SRTM terrain altitude was last updated (msec)
  bool terrain_srtm_alt_valid;  // true when the SRTM terrain altitude is valid (e.g. has been
  ...
  uint32_t prevFlowFuseTime_ms;  // time both flow measurement components passed their innovation
                                 // consistency checks
  Vector2 flowTestRatio;  // square of optical flow innovations divided by fail threshold used by
                          // main filter where >1.0 is a fail
  Vector2F auxFlowTestRatio;  // sum of squares of optical flow innovation divided by fail threshold
                              // used by 1-state terrain offset estimator
  ftype R_LOS;                // variance of optical flow rate measurements (rad/sec)^2
  ftype auxRngTestRatio;  // square of range finder innovations divided by fail threshold used by
                          // main filter where >1.0 is a fail
  Vector2F flowGyroBias;  // bias error of optical flow sensor gyro output
  bool rangeDataToFuse;   // true when valid range finder height data has arrived at the fusion time
                          // horizon.
  bool baroDataToFuse;    // true when valid baro height finder data has arrived at the fusion time
                          // horizon.
  ...
  {
    AID_ABSOLUTE =
      0,  // GPS or some other form of absolute position reference aiding is being used (optical
          // flow may also be used in parallel) so position estimates are absolute.
    AID_NONE = 1,  // no aiding is being used so only attitude and height estimates are available.
                   // Either constVelMode or constPosMode must be used to constrain tilt drift.
    AID_RELATIVE =
      2,  // only optical flow aiding is being used so position estimates will be relative
  };
  AidingMode PV_AidingMode;      // Defines the preferred mode for aiding of velocity and position
                                 // estimates from the INS
  AidingMode PV_AidingModePrev;  // Value of PV_AidingMode from the previous frame - used to detect
                                 // transitions
  bool gndOffsetValid;           // true when the ground offset state can still be considered valid
  Vector3F delAngBodyOF;  // bias corrected delta angle of the vehicle IMU measured summed across
                          // the time since the last OF measurement
  ...
  bool terrainHgtStable;  // true when the terrain height is stable enough to be used as a height
                          // reference
#if EK3_FEATURE_RANGEFINDER_MEASUREMENTS
  ftype storedRngMeas[DOWNWARD_RANGEFINDER_MAX_INSTANCES]
                     [3];  // Ringbuffer of stored range measurements for dual range sensors
  uint32_t storedRngMeasTime_ms[DOWNWARD_RANGEFINDER_MAX_INSTANCES]
                               [3];  // Ringbuffers of stored range measurement times for dual range
                                     // sensors
  uint8_t rngMeasIndex[DOWNWARD_RANGEFINDER_MAX_INSTANCES];  // Current range measurement ringbuffer
                                                             // index for dual range sensors
#endif
...
#endif
  uint32_t lastbodyVelPassTime_ms;  // time stamp when the body velocity measurement last passed
                                    // innovation consistency checks (msec)
  ...
  uint32_t prevBodyVelFuseTime_ms;  // previous time all body velocity measurement components passed
                                    // their innovation consistency checks (msec)
  uint32_t bodyOdmMeasTime_ms;  // time body velocity measurements were accepted for input to the
                                // data buffer (msec)
  ...
  yaw_elements yawAngDataStatic;  // yaw angle (regardless of yaw source) when the vehicle was last
  ...
  {
  public:
    BeaconFusion(AP_DAL & _dal) : dal{_dal}
    {
    }
    ...
    uint32_t lastPassTime_ms;  // time stamp when the range beacon measurement last passed
                               // innovation consistency checks (msec)
    ftype testRatio;           // Innovation test ratio for range beacon measurements
    bool
      health;  // boolean true if range beacon measurements have passed innovation consistency check
    ...
    bool alignmentStarted;  // True when the initial position alignment using range measurements has
                            // started
    bool alignmentCompleted;  // True when the initial position alignment using range measurements
                              // has finished
    uint8_t lastIndex;  // Range beacon index last read -  used during initialisation of the 3-state
                        // filter
    Vector3F posSum;    // Sum of range beacon NED position (m) - used during initialisation of the
                        // 3-state filter
    uint8_t
      numMeas;  // Number of beacon measurements - used during initialisation of the 3-state filter
    ...
    bool
      usingMinHypothesis;  // true when the min beacon constellation offset hypothesis is being used

    ftype posDownOffsetMax;  // Vertical position offset of the beacon constellation origin relative
                             // to the EKF origin (m)
    ...
    ftype posDownOffsetMin;  // Vertical position offset of the beacon constellation origin relative
                             // to the EKF origin (m)
  ...
  uint8_t dragSampleCount;        // number of drag specific force samples accumulated at the filter
                                  // prediction rate
  ftype
    dragSampleTimeDelta;   // time integral across all samples used to form _drag_down_sampled (sec)
  ...
  AP_NavEKF_Source::SourceZ
    prevHgtSource;  // previous height source used to detect changes in source
  ...
  bool finalInflightYawInit;    // true when the final post takeoff initialisation of yaw angle has
                                // been performed
  uint8_t magYawAnomallyCount;  // Number of times the yaw has been reset due to a magnetic anomaly
                                // during initial ascent
  bool finalInflightMagInit;    // true when the final post takeoff initialisation of magnetic field
                                // states been performed
  bool magStateResetRequest;    // true if magnetic field states need to be reset using the
                                // magnetomter measurements
  bool magYawResetRequest;  // true if the vehicle yaw and magnetic field states need to be reset
                            // using the magnetometer measurements
  ...
  ftype yawInnovAtLastMagReset;    // magnetic yaw innovation last time the yaw and mag field states
                                   // were reset (rad)
  ...
  bool dvelBiasAxisInhibit[3]{};  // true when IMU delta velocity bias states for a specific axis is
                                  // de-activated
  ...
  uint32_t extNavMeasTime_ms;  // time external measurements were accepted for input to the data
                               // buffer (msec)
  uint32_t extNavLastPosResetTime_ms;  // last time the external nav systen performed a position
                                       // reset (msec)
  bool extNavDataToFuse;               // true when there is new external nav data to fuse
  bool extNavUsedForPos;  // true when the external nav data is being used as a position reference.
  EKF_obs_buffer_t<ext_nav_vel_elements>
    storedExtNavVel;                      // external navigation velocity data buffer
  ext_nav_vel_elements extNavVelDelayed;  // external navigation velocity data at the fusion time
                                          // horizon.  Already corrected for sensor position
  uint32_t extNavVelMeasTime_ms;   // time external navigation velocity measurements were accepted
                                   // for input to the data buffer (msec)
  ...
  uint32_t extNavVelInnovTime_ms;  // system time that external nav velocity innovations were
                                   // recorded (to detect timeouts)
  ...
  // flags indicating severe numerical errors in innovation variance calculation for different
  // fusion operations
  struct
  {
    bool bad_xmag : 1;
  ...
  */
  ...
  // The following declarations are used to control when the main navigation filter resets it's yaw
  // to the estimate provided by the GSF
  ...
  AP_NavEKF_Source::SourceXY posxy_source_last;  // horizontal position source on previous iteration
                                                 // (used to detect a changes)
  bool posxy_source_reset;  // true when the horizontal position source has changed but the position
                            // has not yet been reset
  AP_NavEKF_Source::SourceYaw
    yaw_source_last;      // yaw source on previous iteration (used to detect a change)
