...
bool NavEKF3_core::healthy(void) const
{
  ...
  // position and height innovations must be within limits when on-ground and in a static mode of
  // operation
...
*/
bool NavEKF3_core::pre_arm_check(
  bool requires_position, char * failure_msg, uint8_t failure_msg_len) const
{
  if (requires_position) {
    ...
    if (
      onGround && PV_AidingMode == AID_ABSOLUTE &&
      ...
      dal.snprintf(
        failure_msg, failure_msg_len, "EKF3[%u] vel error %.1f", unsigned(core_index) + 1,
        hvel_innovation);
      return false;
    }
  }
...
float NavEKF3_core::errorScore() const
{
  ...
  if (tiltAlignComplete && yawAlignComplete) {
    ...
    // Check airspeed fusion performance - only when we are using at least 2 airspeed sensors so we
    // can switch lanes with a better one. This only comes into effect for a forward flight vehicle.
    // A sensitivity factor of 0.3 is added to keep the EKF less sensitive to innovations arising
    // due events like strong gusts of wind, thus, prevent reporting high error scores
    if (assume_zero_sideslip()) {
      const auto * arsp = dal.airspeed();
      if (
        arsp != nullptr && arsp->get_num_sensors() >= 2 &&
        (frontend->_affinity & EKF_AFFINITY_ARSP)) {
        score = MAX(score, 0.3f * tasTestRatio);
      }
    }
    // Check magnetometer fusion performance - need this when magnetometer affinity is enabled to
    // override the inherent compass switching mechanism, and instead be able to move to a better
    // lane
...
bool NavEKF3_core::getHeightControlLimit(float & height) const
{
  ...
  if (
    ...
    (PV_AidingMode == AID_RELATIVE) && flowDataValid) {
    ...
    // if using rangefinder, ensure the height above ground is within range finder limits after
    // accounting for vehicle tilt and control errors
    ...
    return false;
#endif
    // If we are are not using the range finder as the height reference, then compensate for the
    // difference between terrain and EKF origin
...
bool NavEKF3_core::getAirSpdHealthData(
  float & innovation, float & innovationVariance, uint32_t & age_ms) const
{
...
// Return the rate of change of vertical position in the down direction (dPosD/dt) of the body frame
// origin in m/s
float NavEKF3_core::getPosDownDerivative(void) const
{
  ...
  // acceleration correct for the IMU offset (EKF calculations are at the IMU)
  return vertCompFiltState.vel + velOffsetNED.z;
}

// Write the last estimated NE position of the body frame origin relative to the reference point
// (m). Return true if the estimate is valid
bool NavEKF3_core::getPosNE(Vector2p & posNE) const
{
  // There are three modes of operation, absolute position (GPS fusion), relative position (optical
  // flow fusion) and constant position (no position estimate available)
  if (PV_AidingMode != AID_NONE) {
    // This is the normal mode of operation where we can use the EKF position states
    // correct for the IMU offset (EKF calculations are at the IMU)
    posNE = outputDataNew.position.xy().topostype() + posOffsetNED.xy().topostype() +
            public_origin.get_distance_NE_postype(EKF_origin);
    return true;

  } else {
    // In constant position mode the EKF position states are at the origin, so we cannot use them as
    // a position estimate
    if (validOrigin) {
      auto & gps = dal.gps();
      if ((gps.status(selected_gps) >= AP_DAL_GPS::GPS_OK_FIX_2D)) {
        // If the origin has been set and we have GPS, then return the GPS position relative to the
        // origin
  ...
  if (getOriginLLH(origin)) {
    ...
    if (getPosD_local(posD) && PV_AidingMode != AID_NONE) {
      ...
      if (filterStatus.flags.horiz_pos_abs || filterStatus.flags.horiz_pos_rel) {
        ...
        loc.offset(
          outputDataNew.position.x + posOffsetNED.x, outputDataNew.position.y + posOffsetNED.y);
        return true;
      } else {
...
{
  // If in the last 10 seconds we have received flow data and no odometry data, then we are relying
  // on optical flow
  ...
  if (PV_AidingMode == AID_RELATIVE && relyingOnFlowData) {
    ...
    ekfGndSpdLimit = MAX((frontend->_maxFlowRate - 1.0f), 0.0f) *
                     MAX((terrainState - stateStruct.position[2]), rngOnGnd);
...
bool NavEKF3_core::getMagOffsets(uint8_t mag_idx, Vector3f & magOffsets) const
{
  ...
  // compass offsets are valid if we have finalised magnetic field initialisation, magnetic field
  // learning is not prohibited, primary compass is valid and state variances have converged
  const float maxMagVar = 5E-6f;
  bool variancesConverged =
    (P[19][19] < maxMagVar) && (P[20][20] < maxMagVar) && (P[21][21] < maxMagVar);
  if (
    (mag_idx == magSelectIndex) && finalInflightMagInit && !inhibitMagStates &&
    compass.healthy(magSelectIndex) && variancesConverged) {
...
bool NavEKF3_core::getInnovations(
...
// airspeed measurements this indicates the amount of margin available when tuning the various error
// traps also return the delta in position due to the last position reset
bool NavEKF3_core::getVariances(
  ...
  Vector2f & offset) const
{
  ...
  // If we are using simple compass yaw fusion, populate all three components with the yaw test
  // ratio to provide an equivalent output
...
bool NavEKF3_core::getVelInnovationsAndVariancesForSource(
...
{
  faults =
    (stateStruct.quat.is_nan() << 0 | stateStruct.velocity.is_nan() << 1 |
     faultStatus.bad_xmag << 2 | faultStatus.bad_ymag << 3 | faultStatus.bad_zmag << 4 |
     faultStatus.bad_airspeed << 5 | faultStatus.bad_sideslip << 6 | !statesInitialised << 7);
}
...
void NavEKF3_core::send_status_report(GCS_MAVLINK & link) const
{
  ...
  float temp = 0;
  if (
    ((frontend->_useRngSwHgt > 0) && activeHgtSource == AP_NavEKF_Source::SourceZ::RANGEFINDER) ||
    (PV_AidingMode == AID_RELATIVE && flowDataValid)) {
    temp = sqrtF(auxRngTestRatio);
  }

  const mavlink_ekf_status_report_t packet{
    velVar, posVar, hgtVar, fmaxf(fmaxf(magVar.x, magVar.y), magVar.z), temp, flags, tasVar};
...
void NavEKF3_core::getGyroVariance(float var[3]) const
{
  var[0] = P[10][10];
  var[1] = P[11][11];
  var[2] = P[12][12];
...
void NavEKF3_core::getAccelVariance(float var[3]) const
{
  var[0] = P[13][13];
  var[1] = P[14][14];
  var[2] = P[15][15];
