...
const AP_Param::GroupInfo AP_AHRS::var_info[] = {
  ...
  // @Description: This controls how much to use the GPS to correct the attitude. This should never
  // be set to zero for a plane as it would result in the plane losing control in turns. For a plane
  ...
  // @Description: This controls whether to use dead-reckoning or GPS based navigation. If set to 0
  // then the GPS won't be used for navigation, and only dead reckoning will be used. A value of
  // zero should never be used for normal flight. Currently this affects only the DCM-based AHRS:
  // the EKF uses GPS according to its own parameters. A value of 2 means to use GPS for height as
  // well as position - both in DCM estimation and when determining altitude-above-home.
  ...
  // @Param: YAW_P
  // @DisplayName: Yaw P
  // @Description: This controls the weight the compass or GPS has on the heading. A higher value
  ...
  // @Description: This sets the maximum allowable difference between ground speed and airspeed. A
  // value of zero means to use the airspeed as is. This allows the plane to cope with a failing
  // airspeed sensor by clipping it to groundspeed plus/minus this limit. See ARSPD_OPTIONS and
  // ARSPD_WIND_MAX to disable airspeed sensors.
  // @Range: 0 127
  // @Units: m/s
  // @Increment: 1
  // @User: Advanced
  ...
  // @Description: Compensates for the roll angle difference between the control board and the
  ...
  // @Description: Compensates for the pitch angle difference between the control board and the
  ...
  // @Description: Overall board orientation relative to the standard orientation for the board
  // type. This rotates the IMU and compass readings to allow the board to be oriented in your
  // vehicle at any 90 or 45 degree angle. The label for each option is specified in the order of
  // rotations for that orientation. This option takes affect on next boot. After changing you will
  // need to re-level your vehicle. Firmware versions 4.2 and prior can use a CUSTOM (100) rotation
  // to set the AHRS_CUSTOM_ROLL/PIT/YAW angles for AHRS orientation. Later versions provide two
  // general custom rotations which can be used, Custom 1 and Custom 2, with CUST_ROT1_ROLL/PIT/YAW
  // or CUST_ROT2_ROLL/PIT/YAW angles.
  // @Values:
  // 0:None,1:Yaw45,2:Yaw90,3:Yaw135,4:Yaw180,5:Yaw225,6:Yaw270,7:Yaw315,8:Roll180,9:Yaw45Roll180,10:Yaw90Roll180,11:Yaw135Roll180,12:Pitch180,13:Yaw225Roll180,14:Yaw270Roll180,15:Yaw315Roll180,16:Roll90,17:Yaw45Roll90,18:Yaw90Roll90,19:Yaw135Roll90,20:Roll270,21:Yaw45Roll270,22:Yaw90Roll270,23:Yaw135Roll270,24:Pitch90,25:Pitch270,26:Yaw90Pitch180,27:Yaw270Pitch180,28:Pitch90Roll90,29:Pitch90Roll180,30:Pitch90Roll270,31:Pitch180Roll90,32:Pitch180Roll270,33:Pitch270Roll90,34:Pitch270Roll180,35:Pitch270Roll270,36:Yaw90Pitch180Roll90,37:Yaw270Roll90,38:Yaw293Pitch68Roll180,39:Pitch315,40:Pitch315Roll90,42:Roll45,43:Roll315,100:Custom
  // 4.1 and older,101:Custom 1,102:Custom 2
  // @User: Advanced
  AP_GROUPINFO("ORIENTATION", 9, AP_AHRS, _board_orientation, 0),

  // @Param: COMP_BETA
  // @DisplayName: AHRS Velocity Complementary Filter Beta Coefficient
  // @Description: This controls the time constant for the cross-over frequency used to fuse AHRS
  // (airspeed and heading) and GPS data to estimate ground velocity. Time constant is 0.1/beta. A
  // larger time constant will use GPS data less and a small time constant will use air data less.
  // @Range: 0.001 0.5
  // @Increment: 0.01
  // @User: Advanced
  ...
  // @Description: Minimum number of satellites visible to use GPS for velocity based corrections
  // attitude correction. This defaults to 6, which is about the point at which the velocity numbers
  ...
  // @Description: This controls which NavEKF Kalman filter version is used for attitude and
  // position estimation
  ...
  // @Description: Autopilot mounting position roll offset. Positive values = roll right, negative
  // values = roll left. This parameter is only used when AHRS_ORIENTATION is set to CUSTOM.
  // @Range: -180 180
  // @Units: deg
  // @Increment: 1
  // @User: Advanced
  ...
  // @Description: Autopilot mounting position pitch offset. Positive values = pitch up, negative
  // values = pitch down. This parameter is only used when AHRS_ORIENTATION is set to CUSTOM.
  // @Range: -180 180
  // @Units: deg
  // @Increment: 1
  // @User: Advanced
  ...
  // @Description: Autopilot mounting position yaw offset. Positive values = yaw right, negative
  // values = yaw left. This parameter is only used when AHRS_ORIENTATION is set to CUSTOM.
  // @Range: -180 180
  // @Units: deg
  // @Increment: 1
  // @User: Advanced
  ...
  // @Description: This controls optional AHRS behaviour. Setting DisableDCMFallbackFW will change
  // the AHRS behaviour for fixed wing aircraft in fly-forward flight to not fall back to DCM when
  // the EKF stops navigating. Setting DisableDCMFallbackVTOL will change the AHRS behaviour for
  // fixed wing aircraft in non fly-forward (VTOL) flight to not fall back to DCM when the EKF stops
  // navigating. Setting DontDisableAirspeedUsingEKF disables the EKF based innovation check for
  // airspeed consistency. Setting AutoRecordOrigin will auto-save the EKF origin to parameters when
  // it becomes valid.
  // @Bitmask: 0:DisableDCMFallbackFW, 1:DisableDCMFallbackVTOL, 2:DontDisableAirspeedUsingEKF,
  // 3:RecordOrigin, 4:UseRecordedOriginForNonGPS
  // @User: Advanced
...
bool AP_AHRS::_should_use_airspeed_sensor(uint8_t airspeed_index) const
{
  ...
  nav_filter_status filter_status;
  if (
    !option_set(Options::DISABLE_AIRSPEED_EKF_CHECK) && fly_forward && hal.util->get_soft_armed() &&
...
bool AP_AHRS::_airspeed_EAS(
  float & airspeed_ret, AirspeedEstimateType & airspeed_estimate_type) const
{
...
bool AP_AHRS::airspeed_health_data(
...
void AP_AHRS::use_recorded_origin_maybe()
{
  ...
  // try to set origin
  const Location loc{
    int32_t(_origin_lat.get() * 1e7), int32_t(_origin_lon.get() * 1e7),
    int32_t(_origin_alt.get() * 100), Location::AltFrame::ABSOLUTE};
  if (set_origin(loc)) {
    GCS_SEND_TEXT(
      MAV_SEVERITY_INFO, "AHRS: using recorded origin:%.7f,%.7f,%.1f", (double)_origin_lat.get(),
      (double)_origin_lon.get(), (double)_origin_alt.get());
  }
}

#if AP_AHRS_POSITION_RESET_ENABLED
bool AP_AHRS::handle_external_position_estimate(
  const Location & loc, float pos_accuracy, uint32_t timestamp_ms)
{
#if HAL_NAVEKF3_AVAILABLE
...
// get velocity down in m/s.  This returns get_velocity_NED.z() if available, otherwise falls back
// to get_vert_pos_rate_D() if high_vibes is true then this is equivalent to get_vert_pos_rate_D
...
// Get a derivative of the vertical position which is kinematically consistent with the vertical
// position is required by some control loops. This is different to the vertical velocity from the
// EKF which is not always consistent with the vertical position due to the various errors that are
// being corrected for.
...
{
  Location loc;
  if (!_home_is_set || !get_location(loc)) {
    return false;
  }
...
void AP_AHRS::get_relative_position_D_home(float & posD) const
{
  ...
  if (!get_relative_position_D_origin(originD) || !_get_origin(originLLH)) {
    ...
    if (_gps_use == GPSUse::EnableWithHeight && gps.status() >= AP_GPS::GPS_OK_FIX_3D) {
...
AP_AHRS::EKFType AP_AHRS::_active_EKF_type(void) const
{
  ...
  if (_vehicle_class == VehicleClass::FIXED_WING || _vehicle_class == VehicleClass::GROUND) {
    ...
    // Handle fallback for the case where the DCM or EKF is unable to provide attitude or height
    // data.
    const bool can_use_dcm = dcm.yaw_source_available() || fly_forward;
    const bool can_use_ekf =
    ...
    const bool disable_dcm_fallback = fly_forward ? option_set(Options::DISABLE_DCM_FALLBACK_FW)
                                                  : option_set(Options::DISABLE_DCM_FALLBACK_VTOL);
    ...
    if (
      hal.util->get_soft_armed() && (_gps_use != GPSUse::Disable) && should_use_gps &&
      ...
      /*
         ...
         Note: This is a last resort fallback and makes the navigation highly vulnerable to GPS
         noise. Note: When operating in a VTOL flight mode that actively controls height such as
         QHOVER, the EKF gives better vertical velocity and position estimates and height control
         characteristics.
      */
      return EKFType::DCM;
    }
    ...
    if (
      ...
      if (
        (!AP::compass().use_for_yaw()) && AP::gps().status() >= AP_GPS::GPS_OK_FIX_3D &&
  ...
  bool requires_position, char * failure_msg, uint8_t failure_msg_len) const
{
  ...
  if (
    _ekf_type != int8_t(configured_ekf_type()) ||
...
void AP_AHRS::writeBodyFrameOdom(
...
void AP_AHRS::writeExtNavVelData(
...
void AP_AHRS::set_failure_inconsistent_message(
  const char * estimator, const char * axis, float diff_rad, char * failure_msg,
  const uint8_t failure_msg_len) const
{
  hal.util->snprintf(
    failure_msg, failure_msg_len, "%s %s inconsistent %d deg. Wait or reboot", estimator, axis,
    (int)degrees(diff_rad));
}
...
bool AP_AHRS::attitudes_consistent(char * failure_msg, const uint8_t failure_msg_len) const
{
  ...
  if (configured_ekf_type() == EKFType::TWO || active_EKF_type() == EKFType::TWO) {
    for (uint8_t i = 0; i < EKF2.activeCores(); i++) {
      ...
      if (rp_diff_rad > ATTITUDE_CHECK_THRESH_ROLL_PITCH_RAD) {
        set_failure_inconsistent_message(
          "EKF2", "Roll/Pitch", rp_diff_rad, failure_msg, failure_msg_len);
        return false;
      }
  ...
  if (configured_ekf_type() == EKFType::THREE || active_EKF_type() == EKFType::THREE) {
    for (uint8_t i = 0; i < EKF3.activeCores(); i++) {
      ...
      if (rp_diff_rad > ATTITUDE_CHECK_THRESH_ROLL_PITCH_RAD) {
        set_failure_inconsistent_message(
          "EKF3", "Roll/Pitch", rp_diff_rad, failure_msg, failure_msg_len);
        return false;
      }
  ...
  if (!always_use_EKF() || (total_ekf_cores == 1)) {
    ...
    if (rp_diff_rad > ATTITUDE_CHECK_THRESH_ROLL_PITCH_RAD) {
      set_failure_inconsistent_message(
        "DCM", "Roll/Pitch", rp_diff_rad, failure_msg, failure_msg_len);
      return false;
    }
    ...
    using_noncompass_for_yaw =
      (configured_ekf_type() == EKFType::THREE) && EKF3.using_noncompass_for_yaw();
#endif
...
// get_hgt_ctrl_limit - get maximum height to be observed by the control loops in metres and a
// validity flag this is used to limit height during optical flow navigation it will return false
// when no limiting is required
...
// get_variances - provides the innovations normalised using the innovation variance where a value
// of 0 indicates prefect consistency between the measurement and the EKF solution and a value of 1
// is the maximum inconsistency that will be accepted by the filter boolean false is returned if
// variances are not available
bool AP_AHRS::get_variances(
  float & velVar, float & posVar, float & hgtVar, Vector3f & magVar, float & tasVar) const
{
  switch (configured_ekf_type()) {
#if AP_AHRS_DCM_ENABLED
    case EKFType::DCM:
...
// get a source's velocity innovations.  source should be from 0 to 7 (see
// AP_NavEKF_Source::SourceXY) returns true on success and results are placed in innovations and
// variances arguments
bool AP_AHRS::get_vel_innovations_and_variances_for_source(
  uint8_t source, Vector3f & innovations, Vector3f & variances) const
{
  switch (configured_ekf_type()) {
#if AP_AHRS_DCM_ENABLED
    case EKFType::DCM:
    ...
    case EKFType::THREE:
      // use EKF to get variance
      return EKF3.getVelInnovationsAndVariancesForSource(
        (AP_NavEKF_Source::SourceXY)source, innovations, variances);
#endif
...
{
#if defined(AP_AHRS_NAVEKF3_ENABLED) && AP_AHRS_NAVEKF3_ENABLED
  if (active_EKF_type() == EKFType::THREE) {
