...
#define AP_AHRS_NAVEKF_SETTLE_TIME_MS \
  20000  // time in milliseconds the ekf needs to settle after being started
...
public:
  ...
  {
    return state.gyro_estimate;
  }

  // return the current drift correction integrator value
  const Vector3f & get_gyro_drift(void) const
  {
    return state.gyro_drift;
  }
  ...
  // return true if the current AHRS airspeed estimate (from airspeed_estimate method) is directly
  // derived from an airspeed sensor
  ...
  // Set the EKF's NE horizontal position states and their corresponding variances from the supplied
  // WGS-84 location and 1-sigma horizontal position uncertainty. This can be used when the EKF is
  // dead reckoning to periodically correct the position. If the EKF is is still using data from a
  // position sensor such as GPS, the position set will not be performed. pos_accuracy is the
  // standard deviation of the horizontal position uncertainty in metres. The altitude element of
  // the location is not used. Returns true if the set was successful.
  bool handle_external_position_estimate(
    const Location & loc, float pos_accuracy, uint32_t timestamp_);
#endif
  ...
  // vehicle's origin
  bool get_location_from_origin_offset_NED(Location & loc, const Vector3p & offset_ned) const
    WARN_IF_UNUSED;
  bool get_location_from_home_offset_NED(Location & loc, const Vector3p & offset_ned) const
    WARN_IF_UNUSED;
  ...
  // Get a derivative of the vertical position in m/s which is kinematically consistent with the
  // vertical position is required by some control loops. This is different to the vertical velocity
  // from the EKF which is not always consistent with the vertical position due to the various
  ...
  void writeOptFlowMeas(
    ...
    const uint32_t msecFlowMeas, const Vector3f & posOffset, const float heightOverride);
  ...
  // Writes the default equivalent airspeed and its 1-sigma uncertainty in m/s to be used in forward
  ...
  // get_hgt_ctrl_limit - get maximum height to be observed by the control loops in meters and a
  // validity flag this is used to limit height during optical flow navigation it will return
  // invalid when no limiting is required
  ...
  // of 0 indicates perfect consistency between the measurement and the EKF solution and a value of
  // 1 is the maximum inconsistency that will be accepted by the filter boolean false is returned if
  ...
  bool get_vel_innovations_and_variances_for_source(
    uint8_t source, Vector3f & innovations, Vector3f & variances) const WARN_IF_UNUSED;
  ...
  // to use, i.e, the one being used by the primary lane. A lane switch could have happened due to
  // an airspeed sensor fault, which makes this even more necessary
  uint8_t get_active_airspeed_index() const;

  // return the index of the primary core or -1 if no primary core selected
  int8_t get_primary_core_index() const
  {
    return state.primary_core;
  }

  // get the index of the current primary accelerometer sensor
  uint8_t get_primary_accel_index(void) const
  {
    return state.primary_accel;
  }

  // get the index of the current primary gyro sensor
  uint8_t get_primary_gyro_index(void) const
  {
    return state.primary_gyro;
  }
  ...
  {
    return _rotation_autopilot_body_to_vehicle_body;
  }
  ...
  // get the view
  AP_AHRS_View * get_view(void) const
  {
    return _view;
  };
...
private:
  ...
  void calc_trig(
    const Matrix3f & rot, float & cr, float & cp, float & cy, float & sr, float & sp,
    float & sy) const;
  ...
  void update_flags();
  bool takeoff_expected;  // true if the vehicle is in a state that takeoff might be expected.
                          // Ground effect may be in play.
  uint32_t takeoff_expected_start_ms;
  bool touchdown_expected;  // true if the vehicle is in a state that touchdown might be expected.
                            // Ground effect may be in play.
