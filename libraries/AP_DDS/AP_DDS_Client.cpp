...
#if AP_DDS_WHEEL_DATA_PUB_ENABLED
static constexpr uint16_t DELAY_WHEEL_DATA_TOPIC_MS = AP_DDS_DELAY_WHEEL_DATA_TOPIC_MS;
#endif
...
bool AP_DDS_Client::create()
{
  ...
  for (uint16_t i = 0; i < ARRAY_SIZE(topics); i++) {
    ...
    if (topics[i].topic_rw == Topic_rw::DataWriter) {
      ...
      for (uint8_t retry = 0; retry < 3; retry++) {
        success = uxr_run_session_until_all_status(
          &session, requestTimeoutMs * (retry + 1), requests, status, nRequests);
        if (success) {
          break;
        }
        GCS_SEND_TEXT(
          MAV_SEVERITY_WARNING, "%s Topic/Pub/Writer session request retry for index '%u'",
          msg_prefix, i);
      }
      if (!success) {
        GCS_SEND_TEXT(
      ...
      for (uint8_t retry = 0; retry < 3; retry++) {
        ...
        GCS_SEND_TEXT(
          MAV_SEVERITY_WARNING, "%s Topic/Sub/Reader session request retry for index '%u'",
          msg_prefix, i);
      }
      if (!success) {
        GCS_SEND_TEXT(
  ...
  for (uint16_t i = 0; i < ARRAY_SIZE(services); i++) {
    ...
    if (services[i].service_rr == Service_rr::Replier) {
      ...
      for (uint8_t retry = 0; retry < 3; retry++) {
        success = uxr_run_session_until_all_status(
          &session, requestTimeoutMs * (retry + 1), &request, &status, 1);
        if (success) {
          break;
        }
        GCS_SEND_TEXT(
          MAV_SEVERITY_WARNING, "%s Service/Replier session request retry for index '%u'",
          msg_prefix, i);
      }
      if (!success) {
        GCS_SEND_TEXT(
...
void AP_DDS_Client::write_nav_sat_fix_topic()
{
  ...
  if (connected) {
    ...
    uxr_prepare_output_stream(
      &session, reliable_out, topics[to_underlying(TopicIndex::NAV_SAT_FIX_PUB)].dw_id, &ub,
      topic_size);
...
void AP_DDS_Client::write_static_transforms()
{
  ...
  if (connected) {
    ...
    uxr_prepare_output_stream(
      &session, reliable_out, topics[to_underlying(TopicIndex::STATIC_TRANSFORMS_PUB)].dw_id, &ub,
      topic_size);
...
void AP_DDS_Client::write_local_pose_topic()
{
  ...
  if (connected) {
    ...
    uxr_prepare_output_stream(
      &session, reliable_out, topics[to_underlying(TopicIndex::LOCAL_POSE_PUB)].dw_id, &ub,
      topic_size);
...
void AP_DDS_Client::write_tx_local_velocity_topic()
{
  ...
  if (connected) {
    ...
    uxr_prepare_output_stream(
      &session, reliable_out, topics[to_underlying(TopicIndex::LOCAL_VELOCITY_PUB)].dw_id, &ub,
      topic_size);
    const bool success =
...
void AP_DDS_Client::write_tx_local_airspeed_topic()
{
  ...
  if (connected) {
    ...
    uxr_prepare_output_stream(
      &session, reliable_out, topics[to_underlying(TopicIndex::LOCAL_AIRSPEED_PUB)].dw_id, &ub,
      topic_size);
...
void AP_DDS_Client::write_tx_local_rc_topic()
{
  ...
  if (connected) {
    ...
    uxr_prepare_output_stream(
      &session, reliable_out, topics[to_underlying(TopicIndex::LOCAL_RC_PUB)].dw_id, &ub,
      topic_size);
...
void AP_DDS_Client::write_nav_odom_topic()
{
  ...
  if (connected) {
    ...
    uxr_prepare_output_stream(
      &session, best_effort_out, topics[to_underlying(TopicIndex::NAV_ODOM_PUB)].dw_id, &ub,
      topic_size);
...
void AP_DDS_Client::write_geo_pose_topic()
{
  ...
  if (connected) {
    ...
    uxr_prepare_output_stream(
      &session, reliable_out, topics[to_underlying(TopicIndex::GEOPOSE_PUB)].dw_id, &ub,
      topic_size);
...
void AP_DDS_Client::write_gps_global_origin_topic()
{
  ...
  if (connected) {
    ...
    uxr_prepare_output_stream(
      &session, reliable_out, topics[to_underlying(TopicIndex::GPS_GLOBAL_ORIGIN_PUB)].dw_id, &ub,
      topic_size);
    const bool success =
...
void AP_DDS_Client::write_goal_topic()
{
  ...
  if (connected) {
    ...
    uxr_prepare_output_stream(
      &session, reliable_out, topics[to_underlying(TopicIndex::GOAL_PUB)].dw_id, &ub, topic_size);
...
void AP_DDS_Client::write_status_topic()
{
  ...
  if (connected) {
    ...
    uxr_prepare_output_stream(
      &session, reliable_out, topics[to_underlying(TopicIndex::STATUS_PUB)].dw_id, &ub, topic_size);
